...
- (void)initDefaultView {
    ...
    self.navigationController.navigationBar.backgroundColor = [UIColor colorWithRed:74/255.0 green:135/255.0 blue:235/255.0 alpha:1];
...
- (void)showInViewContrller:(UIViewController *)vc choosenNum:(NSInteger)choosenNum delegate:(id<YBImgPickerViewControllerDelegate>)vcdelegate withArr:(NSMutableArray*)picArr withNum:(NSInteger)PhotoNum
{
    pictureArr = [NSMutableArray arrayWithCapacity:10];
    pictureArr = picArr;
    photoCount = 6-pictureArr.count-PhotoNum;
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:rightItemTitle style:UIBarButtonItemStyleDone target:self action:@selector(save)];
    self.navigationItem.rightBarButtonItem.tintColor = [UIColor whiteColor];
    self.delegate = vcdelegate;
    choosenCount = choosenNum;
